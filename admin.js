...
const fpSupabase=window.supabase.createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY);
...
async function isAuthorized(user){
  ...
  const {data,error}=await fpSupabase.from('admin_access').select('user_id').eq('user_id',user.id).maybeSingle();
  return !error&&!!data;
}
...
async function guard(){
  try{
    const {data:{session}}=await fpSupabase.auth.getSession();
...
fpSupabase.auth.onAuthStateChange((event)=>{
  if(event==='SIGNED_OUT')showLogin();
});

async function getProducts(){
  const {data,error}=await fpSupabase.from('products').select('*').order('created_at',{ascending:false});
...
async function render(){
  try{
    ...
    document.querySelectorAll('.delete').forEach(button=>button.addEventListener('click',async()=>{
      ...
      const {error}=await fpSupabase.from('products').delete().eq('id',button.dataset.id);
